    ...
    <div className="p-8">
      <div className="max-w-7xl mx-auto">
        <section className="bg-white border border-gray-300 rounded">
          ...
          <div className="flex gap-4 mb-1 relative">
          <div className="w-[100%]">
            <div className="relative">
              <button
                id="operation-select-custom"
                type="button"
              ...
              >
                ...
                <svg className={`w-5 h-5 ml-3 transition-transform ${showOperationDropdown ? "rotate-180" : "rotate-0"}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              {showOperationDropdown && (
                <div className="absolute z-10 mt-2 w-full bg-white border border-gray-200 rounded-lg shadow-xl overflow-auto max-h-72">
                  ...
                  <ul className="max-h-56 overflow-y-auto">
                    <li className="cursor-pointer px-4 py-2 text-gray-600 text-sm hover:bg-slate-100" onClick={() => { setSelectedOperationId(""); setShowOperationDropdown(false); }}>
                      Sélectionner une opération
                    </li>
                    {filteredOperations.map(op => (
                      <li
                        key={op.Id || op.id}
                        className={`cursor-pointer px-4 py-2 text-gray-900 text-sm hover:bg-indigo-50 ${String(selectedOperationId) === String(op.Id || op.id) ? "bg-indigo-100 font-bold" : ""}`}
                        onClick={() => {
          ...
          {selectedOperationId && (
            <div className="border-b border-gray-300 px-6 py-3 flex justify-between items-center bg-gray-50">
              <div>
                <h3 className="font-bold text-gray-800">Opération: {selectedOperation?.Numero}</h3>
          ...
          <div className="p-6">
            {selectedOperationId ? (
              ...
              <div className="text-center py-10">
                <p className="text-gray-500">Sélectionnez une opération pour voir les membres</p>
              </div>
            )}
          </div>
        </section>
